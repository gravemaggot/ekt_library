        ...
        div(class='q-pa-md')
          q-table(
            ...
            template(v-slot:body-cell-start_date_post="props")
              q-td(align="center")
                | {{ new Date(props.row.readed_posts.find(post => post.user_id === user.id).created_at).toDateString() }}
            // Закончил
            template(v-slot:body-cell-end_date_post="props")
              q-td(align="center")
                | {{ new Date(props.row.readed_posts.find(post => post.user_id === user.id).end_date).toDateString() }}
</template>
    ...
    data() {
      return {
        columns: [
          ...
          { name: 'rating',          label: 'Рейтинг',      align: 'center', field: row => row.calculate_rating,     format: val => '${val}', sortable: true },
